...
def make_gmm_likelihood_schedule() -> GaussianLikelihoodSchedule:
    """Hard-coded level-wise likelihood parameters extracted from the original notebook."""
    means = [
        jnp.array([-12.029783, -11.9742414]),
        jnp.array([-12.0536754, -11.9248266]),
        jnp.array([-12.0566158, -11.8992085]),
        jnp.array([-12.0362732, -11.8858647]),
        jnp.array([-12.0319157, -11.8874174]),
    ]
    diag = [
        2.8296915828537745,
        2.9069127592439767,
...
def main() -> None:
    ...
    parser.add_argument("--out_dir", type=str, default="outputs/gmm")
    args = parser.parse_args()

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    prior = GMMPrior(noise_std=0.4)
    likelihood_schedule = make_gmm_likelihood_schedule()
    ...
    if args.checkpoint:
        # Need a template with the same structure.
    ...
    # Plot: prior vs generated (pre-resample) vs selected (post-resample)
    ...
    # Observation point (you can edit this if desired)
    ...
    plot_final_samples(
        ...
        title=f"GMM prior | telescoping posterior sampling (levels={result.meta['n_levels_run']})",
