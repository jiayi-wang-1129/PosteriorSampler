...
class CTConfig:
    ...
    # Photon statistics.
    n0_photons: float = 5000.0
    background_counts: float = 0.0

    # Ray mask on the (angle, detector) grid.
    # vd_poisson | random | full
    mask_type: str = "full"
    mask_fraction: float = 1              # vd_poisson / random only
    mask_center_fraction: float = 1     # vd_poisson only
    mask_seed: int = 0
...
def ct_config_from_env(env_str, env_int, env_float, img_size: int) -> CTConfig:
    ...
    return CTConfig(
        ...
        num_angles=env_int("CT_NUM_ANGLES", 64),
        num_detectors=env_int("CT_NUM_DETECTORS", 64),
        angle_max_degrees=env_float("CT_ANGLE_MAX_DEGREES", 180.0),
        n0_photons=env_float("CT_N0_PHOTONS", 5000.0),
        background_counts=env_float("CT_BACKGROUND_COUNTS", 0.0),
        mask_type=env_str("CT_MASK_TYPE", "full"),
        mask_fraction=env_float("CT_MASK_FRACTION", 1),
        mask_center_fraction=env_float("CT_MASK_CENTER_FRACTION", 1),
